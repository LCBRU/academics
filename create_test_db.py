...
# Themes
themes = [Theme(name=t) for t in ['Theme 1', 'Theme 2', 'Theme 3']]
db.session.add_all(themes)
db.session.commit()

# Journal
journals = [Journal(name=j) for j in ['Journal 1', 'Journal 2']]
db.session.add_all(journals)
db.session.commit()

# Sponsor
sponsors = [Sponsor(name=s) for s in ['Sponsor 1', 'Sponsor 2']]
...
# Keyword
keywords = [Keyword(keyword=k) for k in ['Keyword 1', 'Keyword 2', 'Keyword 3']]
db.session.add_all(keywords)
db.session.commit()

# Affiliations
affiliations = [Affiliation(name=a) for a in ['Affiliation 1', 'Affiliation 2', 'Affiliation 3', 'Affiliation 4', 'Affiliation 5', 'Affiliation 6']]
...
for _ in range(randint(200, 300)):
    ...
    p = Publication(
        doi=doi,
        nihr_acknowledgement=choice(acknowledgements),
    ...
    cp = CatalogPublication(
        ...
        publication_cover_date=fake.date(),
        ...
        funding_text=fake.paragraph(),
        href=fake.uri(),
        journal=choice(journals),
        is_open_access=choice([True, False]),
    )
    cp.keywords = set(sample(keywords, randint(1, len(keywords))))

    for i, s in enumerate(sample(sources, randint(1, len(sources)))):
